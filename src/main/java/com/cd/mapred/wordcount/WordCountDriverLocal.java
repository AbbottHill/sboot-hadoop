...
//        conf.set("mapreduce.job.jar", "D:\\Idea_Workspace\\sboot-hd\\sboot-hd.jar");
        ...
        // 指定本job工作用到的jar包位置
        wcjob.setJarByClass(WordCountDriverLocal.class);
        // 指定本job用到的mapper类
        ...
        // 指定程序处理到输入数据所在的路径
        FileInputFormat.setInputPaths(wcjob, new Path("D:\\logs\\study\\roll-file-error.log"));

//        FileSystem fileSystem = FileSystem.get(new URI(dfs), conf, "hd");
        FileSystem fileSystem = FileSystem.getLocal(conf);
        Path path = new Path("D:\\logs\\analysis");
